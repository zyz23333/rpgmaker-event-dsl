...
- Decisions: use `audioRef`, `imageRef`, `movieRef`; assets are no-scan.
- Invariants: Asset References never resolve to project data IDs; Project Data References still resolve through normal graph.
...
No asset reference helpers exist yet before Slice 02. `buildSnapshotReferenceInput` already supports some database references but not tilesets; map commands may need additional Project Data Reference kinds if not already present.
...
If a command needs a Project Data Reference kind not currently present, add only the reference kind needed by the MV command and corresponding snapshot extraction if the standard data file is already part of the snapshot model.
