...
import { findMessageBranchEnd, renderInlineCommandListSource } from "../message/decompile.js";
...
export function renderBattleProcessingCommand(
  ...
  while (commands[nextIndex + 1]?.indent === command.indent) {
    ...
    const rendered = renderCommandList(commands.slice(nextIndex + 2, branchEndIndex));
