...
import { renderInlineCommandListSource } from "../message/decompile.js";
...
export function renderConditionalCommand(
  ...
  const renderedThen = renderCommandList(thenCommands);
  ...
  if (commands[thenEndIndex]?.code === 411 && commands[thenEndIndex]?.indent === command.indent) {
    ...
    const renderedElse = renderCommandList(commands.slice(thenEndIndex + 1, elseEndIndex));
...
export function renderLoopCommand(
  ...
  const body = renderCommandList(commands.slice(index + 1, bodyEndIndex));
  const nextIndex =
